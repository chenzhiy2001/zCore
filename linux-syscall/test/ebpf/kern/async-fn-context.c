...
int bpf_prog(struct kprobe_bpf_ctx *ctx) {
  ...
  if (ctx->ptype == 0){
    ...
    u64 depth = bpf_get_depth(ctx->paddr+4 /* annoying math */, ctx->tf.regs[8]);
    bpf_trace_printk("{} 1: [entry] FUNCTION_NAME({}) ",time,ctx->paddr,0);
    bpf_trace_printk("depth: {}\n", depth, 0, 0);
  }
  else{
    bpf_trace_printk("kretprobe@exit", 0, 0, 0);
    u64 time = bpf_ktime_get_ns();
    u64 depth = bpf_get_depth(ctx->paddr /* annoying math */, ctx->tf.regs[8]);
    bpf_trace_printk("{} 1: [exit ] FUNCTION_NAME(FUNCTION_ADDR) ",time,0,0);
    bpf_trace_printk("depth: {}\n", depth, 0, 0);
  }
