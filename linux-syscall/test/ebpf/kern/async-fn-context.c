...
int bpf_prog(struct kprobe_bpf_ctx *ctx) {
  ...
  if (ctx->ptype == 0){
    ...
    u64 depth = bpf_get_depth(ctx->paddr /*+4*/ /*+4 or not? annoying math */, ctx->tf.regs[8]);
    // bpf_trace_printk("{} 1: [entry] FUNCTION_NAME({}) depth: {}\n",time,ctx->paddr,depth);
    // bpf_trace_printk("time-threadID-entry/exit-addr-depth: {} 1 entry {} {}\n",time,ctx->paddr,depth);
    bpf_trace_printk("time-threadID-entry/exit-addr-depth: {}",time,0,0);
    bpf_trace_printk(" {} entry {} {}\n",pid,ctx->paddr,depth);
  }
  else{
    // bpf_trace_printk("kretprobe@exit", 0, 0, 0);
    u64 time = bpf_ktime_get_ns();
    u64 depth = bpf_get_depth(ctx->paddr /*+4 or not? annoying math */, ctx->tf.regs[8]);
